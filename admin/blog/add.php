...
if (isset($_SESSION['user'])) {
    ...
    if (mysqli_num_rows($check_user) == 0) {
        ...
        if (isset($_POST['add'])) {
            ...
            $post_url = generateUrl($post_judul);

            if (empty($post_judul)) {
                $msg_type = "error";
                $msg_content = "Please Fill In All Inputs.";
            } else {
                 $insert_news = mysqli_query($db, "INSERT INTO blog (id, judul, image, konten, slider, url) VALUES ('','$post_judul', '$post_image', '$post_konten','$post_slider', '$post_url')");
                if ($insert_news) {
                    $msg_type = "success";
