...
if (isset($_SESSION['user'])) {
	...
	if (mysqli_num_rows($check_user) == 0) {
		...
		if (isset($_POST['delete'])) {
			$post_id = $_POST['id'];
			$checkdb_news = mysqli_query($db, "SELECT * FROM livetv WHERE id = '$post_id'");
			if (mysqli_num_rows($checkdb_news) == 0) {
				$msg_type = "error";
				$msg_content = "News Cannot Be Found.";
			} else {
				$delete_news = mysqli_query($db, "DELETE FROM livetv WHERE id = '$post_id'");
				if ($delete_news == TRUE) {
					$msg_type = "success";
...
<html
  ...
  <head>
    ...
    <title>Halaman Live Tv | <?php echo $data_settings['web_name']; ?></title>
          ...
          <div class="content-wrapper">
            ...
            <div class="container-xxl flex-grow-1 container-p-y">
              ...
              <a href="add" class="btn rounded-pill btn-primary">
                  <span class="ti-xs ti ti-plus"></span>Tambah
              </a>
            </div>
            <br>
              ...
              <div class="card">
                <div class="card-datatable table-responsive pt-0">
                  <table id="datasosmed" class="table table-striped" style="width:100%">
                    ...
                    <?php
							// start paging config
							$query_list = mysqli_query($db, "SELECT * FROM livetv ORDER BY id DESC"); // edit
							$no = 1;
							// end paging config
                        ...
                        <tr>
                            ...
                            <td><?php echo $data_show['judul']; ?></td>
                            <td><?php echo nl2br(substr($data_show['konten'], 0, 50)); ?></td>
                            <td><?php echo $data_show['waktu']; ?>/<?php echo $data_show['tanggal']; ?></td>
                            <td><?php echo $data_show['slider']; ?></td>
                            <td>
