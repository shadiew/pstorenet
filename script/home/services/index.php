...
<body class="apps-craft-solid-color apps-craft-particle">
    ...
    <div class="container panel panel-default table-box-services">                                                                                   
  <div class="table-responsive">
  <table class="table table-bordered table-services table-hover m-0">
												<thead>
													<tr>
														<th>ID</th>
														<th>Service</th>
												...
												<tbody>
																								<?php
$ca_query_list = "SELECT * FROM service_cat WHERE status = 'Active' ORDER BY id ASC";
$ca_new_query = $ca_query_list;
$ca_new_query = mysqli_query($db, $ca_new_query);
    
    while ($ca_data_service = mysqli_fetch_assoc($ca_new_query)) {
        $cat = $ca_data_service['name'];
    ?>
        <tr>
            <td class="td-services-cat" bgcolor="#2690FB" colspan="6"><b><?php echo $cat; ?></b></td>
        <tr>

        <?php

            // start paging config
            $query_list = "SELECT * FROM services WHERE category = '$cat' AND status = 'Active' ORDER BY sid ASC";
...
// end paging config
												...
												?>
													<tr>
                                                        ...
                                                        <td><div class="eye-symbol" onclick="<?php echo htmlspecialchars("swal(\"".$data_service['service']."\", ".json_encode($data_service['note']).")"); ?>"><i class="zmdi zmdi-eye zmdi-hc-fw eye-blue"></i></div></td>
	</tr>
	<?php } ?>
    <?php } ?>
												</tbody>
