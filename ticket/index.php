...
if (isset($_POST['submit'])) {
    ...
    if (empty($post_subject) || empty($post_message)) {
        $msg_type = "error";
        $msg_content = "Please fill all input.";
    } else if (strlen($post_subject) > 200) {
        $msg_type = "error";
        $msg_content = "Maximum subject is 200 characters.";
        ...
        $msg_type = "error";
        $msg_content = "Minimum Message Is 20 Characters.";
    } else {
        $ip = $_SERVER['REMOTE_ADDR'];
        ...
        if ($insert_ticket == TRUE) {
            $msg_type = "success";
            $msg_content = "Ticket Has Been Sent.";
        } else {
            $msg_type = "error";
...
$check_user = mysqli_query($db, "SELECT * FROM users WHERE username = '$sess_username'");
$data_user = mysqli_fetch_assoc($check_user);
$lastUpdate = new DateTime($data_show['last_update']);
$formattedDate = $lastUpdate->format('d F Y');
$formattedTime = $lastUpdate->format('H:i:s');
...
<html
  ...
  <head>
    ...
    <title>Tiket Bantuan | <?php echo $data_settings['web_name']; ?></title>
                            ...
                            <div class="modal fade" id="addNewCCModal" tabindex="-1" aria-hidden="true">
                                ...
                                <div class="modal-content p-3 p-md-5">
                                    ...
                                    <div class="text-center mb-4">
                                        <h3 class="mb-2">Buat Ticket Baru</h3>
                                        <p class="text-muted">Kami akan selalu membantu anda</p>
                                    </div>
                                    <form  class="row g-3" method="POST" action="<?php echo $_SERVER['PHP_SELF']; ?>">
                                        <div class="col-12">
                                        <label class="form-label w-100">Subjek</label>
                                        ...
                                        <div class="col-12">
                                            <label class="form-label" for="modalAddCardName">Pesan Anda</label>
                  ...
                  <table id="datasosmed" class="table table-striped" style="width:100%">
                    ...
                    <?php
                        ...
                        <tr>
                            ...
                            <td><span class="badge bg-<?php echo $label; ?>"><?php echo $data_show['status']; ?></span></td>
                            <td><?php if($data_show['seen_user'] == 0) { ?><label class="badge badge">NEW!</label><?php } ?> <a href="open/?id=<?php echo $data_show['id']; ?>"><?php echo $data_show['subject']; ?></a></td>
