...
if (mysqli_num_rows($check_order) == 0) {
	...
	while($data_order = mysqli_fetch_assoc($check_order)) {
		...
		$u_remains = $data_order['remains'];
    		...
    		$update_order = mysqli_query($db, "INSERT INTO balance_history (username, action, quantity, clearance, msg, date, time, type) VALUES ('$buyer', 'Add Balance', '$refund', '$$temp_balance', 'Order Refund, ID : $o_oid', '$date', '$time', '+ $')");
    		if ($update_order == TRUE) {
    			echo "$o_oid - $$refund - Has been Refunded<br />";
    		} else {
    			echo "Error database.";
    		}
	}
